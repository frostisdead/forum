...
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-md w-full space-y-8">
                ...
                <form class="mt-8 space-y-6" action="{{route('custom.create.category')}}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="remember" value="true">
                    <div class="rounded-md shadow-sm -space-y-px">
                        <div>
                            <label for="category_name" class="sr-only">Category name</label>
                            <input id="category_name" name="category_name" type="text" autocomplete="category_name" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Category name">
                        </div>
                        <div>
